...
const Header = () => {
  ...
  async function onLogout() {
    ...
    navigate("/auth"); // Redirect using navigate
  }
  ...
  return (
    <div className="flex justify-around p-3 border-b-2 sticky top-0 bg-white">
      <div className="flex gap-8 items-center">
        ...
        <button className="font-semibold">TOEFL</button>
        <button className="font-semibold">IELTS</button>
        <button className="font-semibold border-r-2 border-gray-300 w-28">
          Digital SAT
        </button>
        <button>TUTOR</button>
        <button>TG Study Abroad</button>
      </div>
      <div>
        {login ? (
          <button
